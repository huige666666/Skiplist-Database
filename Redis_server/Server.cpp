...
void *WorkerThread(void *arg) {
	...
	while (true&&c->connfd) {
		...
		ret = recv(c->connfd, szMessage, MSGSIZE, 0);
		if (ret <= 0)return 0;
		szMessage[ret] = '\0';
		...
		if (C._arg[0] == "set") {
			...
			send(c->connfd, str, strlen(str) + sizeof(char),0);
			continue;
...
int main()
{
	...
	pthread_t *m_threads; 
	m_threads = new pthread_t[MAXCLIENTNUM];
	...
	for (int i = 0; i < MAXCLIENTNUM; i++) {
		...
		//client->sClient = accept(server->sListen, (struct sockaddr*)&(client->my_client), &iaddrSize);
         ...
         client->connfd = accept(listenfd, (struct sockaddr *)&client_address, &client_addrlength);
         if (client->connfd < 0)
            {
		...
		//hThread[i] = CreateThread(NULL, 0, WorkerThread, client, 0, NULL);
		 if (pthread_create(m_threads + i, NULL, WorkerThread, client) != 0)
        {
            delete[] m_threads;
            throw std::exception();
        }
